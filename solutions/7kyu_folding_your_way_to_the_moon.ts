...
export const foldTo = (distance: number): number | null => {
  const thicknessOfPaper = 0.0001;
  if (distance < 0) {
    return null;
  }
  if (distance < thicknessOfPaper) {
    return 0;
  }
  const folds = Math.ceil(Math.log2(distance / thicknessOfPaper));
  return folds;
};
